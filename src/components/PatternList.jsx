...
const PatternList = ({ patterns, user, onApply }) => {
  console.log("PatternList received patterns:", patterns); // Debugging
  ...
  const handleApply = async (patternId) => {
    ...
    try {
      ...
      // Notify parent (TesterDashboard) to update selected pattern or refresh
      const appliedPattern = patterns.find((p) => p._id === patternId);
      if (appliedPattern) {
        onApply({
          ...appliedPattern,
          applicants: [...appliedPattern.applicants, user._id],
        });
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      {patterns.length === 0 ? (
        ...
        patterns.map((pattern) => {
          const hasApplied = pattern.applicants.some(
